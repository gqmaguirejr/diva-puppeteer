...
async function extractOrganizationInformation(page, blanks) {
	...
	selector1=make_selector_from_id(iIDs[i])+' a';
	console.log("selector1 is ", selector1);
	await page.click(selector1);
	already_clicked[selector1]=true;

	selector0='organisationPopupForm:tree-d-'+i+'-c';
	...
	for (let j = 0; j < divs1.length; j++) {
	    ...
	    selector2=make_selector_from_id(iIDs[j])+' a';
	    console.log("selector2 is ", selector2);
	    //if (already_clicked.hasOwnProperty(selector2)) {
	    //	} else {
		await page.click(selector2);
	    // already_clicked[selector2]=true;
	    //}
	    // selector0='organisationPopupForm:tree-d-'+i+'-'+j+'-c';
	    // console.log("selector0 is ", selector0);
	    // await page.waitForSelector(make_selector_from_id(selector0), { timeout: 1000 })
	    ...
	    // 	org3=textDiv1[j].trim();
	    // 	index3=iIDs1[j].slice(dPrefix.length).split(':')[0]; // as the Nordic Institute for Theoretical Physics NORDITA index2 is 0-0:j_id202 -- rmove the trailing ":j_id202"
	    // 	console.info("org3 is ", org3, "index3 is", index3);
	    // 	kth_org[org1][org2][org3]={};
	    // 	kth_org[org1][org2][org3]['ID']=index2;
	    // }
	}
    }
