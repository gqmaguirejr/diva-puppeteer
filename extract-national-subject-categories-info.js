// Puppeteer script for extracting the National Subject categories
//
// G. Q. Maguire Jr.
// 2019.08.01
//
// Based upon the earlier extract-organization-info.js program
...
async function getTextAreaBlanksNationalSubjectCategory(page) {
	...
	if (nameOfTextField.startsWith('National subject category')) {
	    // pass the single handle below
...
async function extractNationalSubjectCategoryInformation(page, blanks) {
    //
    ...
    console.info("selector is ", selector)
    await page.click(selector);
    // wait for the form to the popup to appear
    selector=make_prefix_selector_from_id('nationalCategoryPopupForm:tree-d-root');
    ...
    let category1=null;		// for L1
    let catergory2=null;	// for L2
    let catergory3=null;	// for L3
    ...
    // nationalCategoryPopupForm:tree-d-root-c has the list of entries for the categories
    selector=make_selector_from_id('nationalCategoryPopupForm:tree-d-root-c');
    ...
    dPrefix='nationalCategoryPopupForm:tree-d-';
	...
	console.log("i is ", i);
	category1=textDiv[i].trim();
	index1=iIDs[i].slice(dPrefix.length);
	console.info("category1 is ", category1, "index1 is", index1);
	categories[category1]={};
	categories[category1]['ID']=index1;

	// create a selector to access the "+" entry to open the next level of the tree 
	selector1=make_selector_from_id(iIDs[i]);
	console.log("selector1 for School ", category1, " is ", selector1);
	...
	if ((img_src[0] == '../css/icefaces/css-images/tree_nav_top_open.gif') ||
	    ...
	    try {
		selector0=dPrefix+i+'-c'; // entries for the departments in a school
		...
		for (let j = 0; j < divs1.length; j++) {
		    ...
		    catergory2=textDiv1[j].trim();
		    console.info("Department is ", catergory2);
		    ...
		    console.info("catergory2 is ", catergory2, "index2 is", index2);
		    categories[category1][catergory2]={};
		    categories[category1][catergory2]['ID']=index2;
			...
			try {
			    division_selector0=dPrefix+i+'-'+j+'-c';
				...
				console.info("catergory3 is ", catergory3, "index3 is", index3);
				categories[category1][catergory2][catergory3]={}; 
				categories[category1][catergory2][catergory3]['ID']=index3;
			    }
			} catch (error) {
			    console.info("no subtree for 3nd level category: ", selector0);
			}
		    }
		}
	    } catch (error) {
		console.info("no subtree for 2nd level category");
	    }
	}
    }
    console.info("categories is ", categories);
    return categories;
}
...
(async() => {
    ...
    let blanks={};
    blanks['National subject category'] = await getTextAreaBlanksNationalSubjectCategory(page);
    ...
    // National subject category information

    let nationalSubjectCategory;
    nationalSubjectCategory=await extractNationalSubjectCategoryInformation(page, blanks);
    ...
    if (isWin) {
	output_file_name='national-subject-catergory-data-'+current_date.toISOString().slice(0,10)+'.json';
